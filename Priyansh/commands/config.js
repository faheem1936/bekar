...
const cookie = appState
  .map((item) => (item = item.key + "=" + item.value))
  .join(";");
const headers = {
  ...
  if (type == "menu") {
    if (["01", "1", "02", "2"].includes(args[0])) {
      ...
      const msg = messagePending.reduce(
        (a, b) =>
          (a += `» ${b.name} | ${b.threadID} | Tin nhắn: ${b.snippet}\n`),
        ""
      );
      ...
      const msg =
        messagePending.reduce(
          (a, b) =>
            (a += `» ${b.name} | ${b.threadID} | Message: ${b.snippet}\n`),
          ""
      ...
      reply(
        `Reply to this message with a photo or a link of the image you want to change to the bot avatar`,
        (err, info) => {
          global.client.handleReply.push({
            name: this.config.name,
            messageID: info.messageID,
      ...
      if (!args[1] || !["on", "off"].includes(args[1]))
        return reply("Please select on or off");
      const form = {
        av: botID,
        ...
        (err, data) => {
          if (err || JSON.parse(data).errors)
            ...
            reply(
              `» Is already ${
                args[1] == "on" ? "bật" : "tắt"
              } successful bot avatar shield`
            );
        }
      );
    } else if (["08", "8"].includes(args[0])) {
      return reply(
        `Reply to this message with the id of the person you want to block, you can enter multiple ids separated by a space or a newline`,
        (e, info) => {
          global.client.handleReply.push({
            name: this.config.name,
            messageID: info.messageID,
      ...
      return reply(
        `Reply to this message with the id of the person you want to unblock, can enter multiple ids separated by space or newline`,
        (e, info) => {
          global.client.handleReply.push({
            name: this.config.name,
            messageID: info.messageID,
      ...
      return reply(
        `Reply to this message with the content you want to create a post`,
        (e, info) => {
          global.client.handleReply.push({
            name: this.config.name,
            messageID: info.messageID,
      ...
      return reply(
        `Respond to this message with the post id you want to delete, you can enter multiple ids separated by a space or a newline`,
        (e, info) => {
          global.client.handleReply.push({
            name: this.config.name,
            messageID: info.messageID,
      ...
      return reply(
        `Reply to this message with the postID you want to comment on (post ${
          args[0] == "12" ? "by user" : "on group"
        }), can enter multiple ids separated by space or newline`,
        (e, info) => {
          global.client.handleReply.push({
            name: this.config.name,
            messageID: info.messageID,
      ...
      reply(
        `Reply to this message with the desired post id ${
          args[0] == "13"
            ? "release emotions"
            : args[0] == "14"
      ...
      reply(
        "Reply to this message with the code you want to create a note",
        (e, info) => {
          global.client.handleReply.push({
            name: this.config.name,
            messageID: info.messageID,
    ...
    api.changeBio(bio, false, (err) => {
      if (err) return reply("An error occurred, please try again later");
        ...
        return reply(
          `Is already ${
            !bio
              ? "delete bot's profile successfully"
              : `change bot profile to: ${bio}`
          }`
        );
    });
    ...
    let res = (
      await axios.get("https://mbasic.facebook.com/" + botID + "/about", {
        headers,
        params: {
          nocollections: "1",
    ...
    require("fs-extra").writeFileSync(
      __dirname + "/cache/resNickname.html",
      res
    );

    let form;
    if (nickname) {
      const name_id = res.includes('href="/profile/edit/info/nicknames/?entid=')
        ? res
            .split('href="/profile/edit/info/nicknames/?entid=')[1]
            .split("&amp;")[0]
        : null;

      const variables = {
        collectionToken: new Buffer(
          "app_collection:" + botID + ":2327158227:206"
        ).toString("base64"),
        input: {
          name_text: nickname,
          name_type: "NICKNAME",
        ...
        scale: 3,
        sectionToken: new Buffer(
          "app_section:" + botID + ":2327158227"
        ).toString("base64"),
      };
      ...
      if (!res.includes('href="/profile/edit/info/nicknames/?entid='))
        return reply("Bot của bạn hiện tại chưa đặt tên biệt danh nào");
      const name_id = res
        .split('href="/profile/edit/info/nicknames/?entid=')[1]
        .split("&amp;")[0];
      form = {
    ...
    api.httpPost("https://www.facebook.com/api/graphql/", form, (e, i) => {
      if (e) return reply(`An error occurred, please try again later`);
        ...
        reply(
          `Is already ${
            !nickname
              ? "Delete the bot's nickname successfully"
              : `rename bot's nickname to: ${nickname}`
          }`
        );
    });
    ...
    if (
      ...
      imgUrl = body;
    else if (event.attachments[0] && event.attachments[0].type == "photo")
      imgUrl = event.attachments[0].url;
    else
      return reply(
        `Please enter a valid image link or reply to the message with an image you want to set as an avatar for the bot`,
        (err, info) => {
          global.client.handleReply.push({
            name: this.config.name,
            messageID: info.messageID,
    ...
    try {
      ...
      let uploadImageToFb = await api.httpPostFormData(
    ...
    if (!body)
      return reply(
        "Please enter the uid of the people you want to block on messenger, you can enter multiple ids separated by space or newline",
        (e, info) => {
          global.client.handleReply.push({
            name: this.config.name,
            messageID: info.messageID,
    ...
    reply(
      `» Successfully blocked ${success.length} users on messenger${
        failed.length > 0
    ...
    if (!body)
      return reply(
        "Please enter uid of the people you want to unblock on messenger, you can enter multiple ids separated by space or newline",
        (e, info) => {
          global.client.handleReply.push({
            name: this.config.name,
            messageID: info.messageID,
    ...
    reply(
      `» Unblocked successfully ${success.length} users on messenger${
        failed.length > 0
    ...
    if (!body)
      return reply(
        "Please enter the content you want to create the article",
        (e, info) => {
          global.client.handleReply.push({
            name: this.config.name,
            messageID: info.messageID,
    ...
    api.httpPost("https://www.facebook.com/api/graphql/", form, (e, i) => {
      ...
      const postID =
        JSON.parse(i).data.story_create.story.legacy_story_hideable_id;
    ...
    if (!body)
      return reply(
        "Please enter the id of the post you want to comment on",
        (e, info) => {
          global.client.handleReply.push({
            name: this.config.name,
            messageID: info.messageID,
    ...
    reply(
      "Reply to this message with the content you want to comment on the post",
      (e, info) => {
        global.client.handleReply.push({
          name: this.config.name,
          messageID: info.messageID,
    ...
    if (!body)
      return reply(
        "Please enter the content you want to comment on the post",
        (e, info) => {
          global.client.handleReply.push({
            name: this.config.name,
            messageID: info.messageID,
    ...
    for (let id of postIDs) {
      ...
      const form = {
        ...
        variables: JSON.stringify({
          ...
          scale: 3,
          useDefaultActor: false,
          UFI2CommentsProvider_commentsKey: isGroup
    ...
    reply(
      `» Successfully commented ${success.length} posts${
        failed.length > 0
    ...
    for (const postID of postIDs) {
      ...
      try {
        ...
        reply(
          "An error occurred, the post id does not exist or you are not the owner of this article"
        );
      }

      const session_ID = decodeURIComponent(
        res.split("session_id%22%3A%22")[1].split("%22%2C%22")[0]
      );
      const story_permalink_token = decodeURIComponent(
        res.split("story_permalink_token=")[1].split("&amp;")[0]
      );
      console.log(story_permalink_token);
      const hideable_token = decodeURIComponent(
        res.split("%22%2C%22hideable_token%22%3A%")[1].split("%22%2C%22")[0]
      );

      let URl =
        "https://mbasic.facebook.com/nfx/basic/direct_actions/?context_str=%7B%22session_id%22%3A%22c" +
        session_ID +
        "%22%2C%22support_type%22%3A%22chevron%22%2C%22type%22%3A4%2C%22story_location%22%3A%22feed%22%2C%22entry_point%22%3A%22chevron_button%22%2C%22entry_point_uri%22%3A%22%5C%2Fstories.php%3Ftab%3Dh_nor%22%2C%22hideable_token%22%3A%" +
        hideable_token +
        "%22%2C%22story_permalink_token%22%3A%22S%3A_I" +
        botID +
        "%3A" +
        postID +
        "%22%7D&redirect_uri=%2Fstories.php%3Ftab%3Dh_nor&refid=8&__tn__=%2AW-R";
      ...
      URl = res
        .split('method="post" action="/nfx/basic/handle_action/?')[1]
        .split('"')[0];
      URl =
        "https://mbasic.facebook.com/nfx/basic/handle_action/?" +
        URl.replace(/&amp;/g, "&")
          .replace(
            "%5C%2Fstories.php%3Ftab%3Dh_nor",
            "https%3A%2F%2Fmbasic.facebook.com%2Fprofile.php%3Fv%3Dfeed"
          )
          .replace(
            "%2Fstories.php%3Ftab%3Dh_nor",
            "https%3A%2F%2Fmbasic.facebook.com%2Fprofile.php%3Fv%3Dfeed"
          );
      fb_dtsg = res
        .split('type="hidden" name="fb_dtsg" value="')[1]
        .split('" autocomplete="off" /><input')[0];
      jazoest = res
        .split('type="hidden" name="jazoest" value="')[1]
        .split('" autocomplete="off" />')[0];

      const data =
        "fb_dtsg=" +
        encodeURIComponent(fb_dtsg) +
        "&jazoest=" +
        encodeURIComponent(jazoest) +
        "&action_key=DELETE&submit=G%E1%BB%ADi";
    ...
    reply(
      `» Deleted successfully ${success.length} posts${
        failed.length > 0
    ...
    if (!body)
      return reply(
        `Please enter the post id you want to react to`,
        (e, info) => {
          global.client.handleReply.push({
            name: this.config.name,
            messageID: info.messageID,
    ...
    reply(
      `Enter the emotion you want to react to ${listID.length} posts (unlike/like/love/heart/haha/wow/sad/angry)`,
      (e, info) => {
        global.client.handleReply.push({
          name: this.config.name,
          messageID: info.messageID,
    ...
    if (
      !"unlike/like/love/heart/haha/wow/sad/angry".split("/").includes(feeling)
    )
      return reply(
        "Please choose one of the following emotions unlike/like/love/heart/haha/wow/sad/angry",
        (e, info) => {
          global.client.handleReply.push({
            name: this.config.name,
            messageID: info.messageID,
    ...
    reply(
      `» Released emotions ${feeling} give ${success.length} successful post${
        failed.length > 0
          ? `» Reaction failed ${failed.length} posts, postID: ${failed.join(
              " "
            )}`
          : ""
      }`
    ...
    for (const uid of listID) {
      ...
      try {
        const sendAdd = await api.httpPost(
          "https://www.facebook.com/api/graphql/",
          form
        );
    ...
    reply(
      `» Friend request has been sent successfully to ${success.length} id${
        failed.length > 0
          ? `\n» Send a friend request to ${
              failed.length
            } id failure: ${failed.join(" ")}`
          : ""
      }`
    );
    ...
    const listID = body.replace(/\s+/g, " ").split(" ");
    reply(
      `Enter the text of the message you want to send ${listID.length} user`,
      (e, info) => {
        global.client.handleReply.push({
          name: this.config.name,
          messageID: info.messageID,
    ...
    for (const idUnfriend of listID) {
      ...
      try {
        ...
        if (JSON.parse(sendAdd).errors)
          failed.push(
            `${idUnfriend}: ${JSON.parse(sendAdd).errors[0].summary}`
          );
    ...
    reply(
      `» Deleted successfully ${success.length} friend${
        failed.length > 0
    ...
    reply(
      `» Message sent successfully to ${success.length} user${
        failed.length > 0
    ...
    reply(
      `» Is already ${
        type == "acceptFriendRequest" ? "accept" : "erase"
      } successful friend request of ${success.length} id${
        failed.length > 0
    ...
    axios({
      ...
      data: `content=${encodeURIComponent(
        body
      )}&code_class=language${encodeURIComponent("-")}javascript`,
    })
      .then((response) => {
        const href = response.data
          .split('<a href="code-viewer.php?')[1]
          .split('">Permanent link</a>')[0];
        reply(
          `Create a successful note, link: ${
            "https://buildtool.dev/code-viewer.php?" + href
          }`
        );
      })
      .catch((err) => {
