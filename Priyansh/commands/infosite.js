...
module.exports.run = async ({ api, event, args }) => {
  ...
  var juswa = args.join(" ");
  if (!juswa)
    return api.sendMessage(`add url site`, event.threadID, event.messageID);
  else {
    axios
      .get(`https://list.ly/api/v4/meta?url=${encodeURIComponent(juswa)}`)
      .then((res) => {
        let a = res.data.name,
        ...
        request(encodeURI(c))
          .pipe(fs.createWriteStream(__dirname + `/cache/juswa.png`))
          .on("close", callback);
      });
  }
};
