...
module.exports.run = async ({ api, event, args }) => {
  ...
  if (args[0] == "admin") {
    ...
    return request(
      encodeURI(
        `https://graph.facebook.com/100012191281263/picture?height=720&width=720&access_token=6628568379%7Cc1e620fa708a1d5696fb991c1bde5662`
      )
    )
      .pipe(fs.createWriteStream(__dirname + "/cache/1.png"))
      .on("close", () => callback());
  ...
  if (args[0] == "user") {
    if (!args[1]) {
      ...
      let b =
        data[id].isFriend == false
          ? "are not !"
          : data[id].isFriend == true
          ? "Yes !"
          : "Damn";
