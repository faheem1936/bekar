  ...
  try {
    ...
    if (event.attachments.length != 0) {
      for (var p of event.attachments) {
        ...
        let m = (
          await axios.get(encodeURI(p.url), { responseType: "arraybuffer" })
        ).data;
            ...
            api.sendMessage(
              {
                body:
                  "[📲] Feedback from " + name + ":\n" + event.body ||
                  "only files that don't have a reply 🧡",
                attachment: l,
                mentions: [
                  {
                    id: event.senderID,
      ...
      case "calladmin": {
        if (s.length == 0) {
          api.sendMessage(
            {
              body: `[📌] Feedback from admin ${name} to you:\n\n[💬] Content: ${
                event.body || "no reply 🌸 "
              }\n\n» Reply to this message if you want to continue sending  reports to my owner 𝐏𝐫𝐢𝐲𝐚𝐧𝐬𝐡 𝐑𝐚𝐣𝐩𝐮𝐭`,
          ...
          api.sendMessage(
            {
              body: `[📌] Feedback from admin ${name} to you:\n\n[💬] Content: ${
                event.body || "only files with no reply 🌸 "
  ...
  try {
    ...
    if (!args[0] && event.messageReply.attachments.length == 0)
      return api.sendMessage(
        `You haven't entered what to report 📋`,
        event.threadID,
        event.messageID
      );
    ...
    api.sendMessage(
      `[🤖] - Bot has just successfully sent your message to ${soad} to my owner Faheem Akhtar🍄 \n[⏰] - Time: ${gio}`,
      event.threadID,
      () => {
        var idad = global.config.ADMINBOT;
        if (s.length == 0) {
