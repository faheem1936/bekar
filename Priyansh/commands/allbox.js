module.exports.config = {
  ...
  hasPermssion: 2,
  description:
    "[Ban/Unban/Del/Remove] List[Data] thread The bot has joined in.",
    ...
    case "reply": {
      if (arg[0] == "ban" || arg[0] == "Ban") {
        ...
        return api.sendMessage(
          `»Notifications from Owner Faheem«\n\n Group of Friends Have been banned from using bots by Ban.`,
          idgr,
          () =>
      ...
      if (
        ...
        return api.sendMessage(
          `»Notifications from Owner Faheem«\n\n Group Of Friends That Have Been Removed Board`,
          idgr,
          () =>
            api.sendMessage(`${api.getCurrentUserID()}`, () =>
              api.sendMessage(
...
module.exports.run = async function ({ api, event, args }) {
  switch (args[0]) {
    case "all":
      {
        ...
        api.sendMessage(
          msg +
            "🎭Reply Out, Ban, Unban, Del[data] the order number to Out, Ban, Unban, Del[data] that thread!",
          event.threadID,
          (e, data) =>
    ...
    default:
      ...
      for (const thread of data) {
        var nameThread =
          (await global.data.threadInfo.get(thread).threadName) ||
