...
module.exports.run = async ({ event, api, args }) => {
  ...
  try {
    if (global.client.family == true)
      return api.sendMessage(
        "The system is processing a request from another box, please come back later",
        event.threadID,
        event.messageID
      );
    ...
    if (!fs.existsSync(__dirname + "/cache/VNCORSI.ttf")) {
      let getfont = (
        await axios.get(
          `https://drive.google.com/uc?id=1q0FPVuJ-Lq7-tvOYH0ILgbjrX1boW7KW&export=download`,
          { responseType: "arraybuffer" }
        )
      ).data;
      fs.writeFileSync(
        __dirname + "/cache/VNCORSI.ttf",
    ...
    const background = await Canvas.loadImage(
      "https://i.ibb.co/QvG4LTw/image.png"
    );
    ...
    if (s > ybground || s > xbground) {
      global.client.family = false;
      return api.sendMessage(
        `Size avatar phải nhỏ hơn size background\nSize background: X: ${xbground}, Y: ${ybground}`,
        threadID,
        messageID
      );
    }
    api.sendMessage(
      `🔢Estimated number of photos: ${idtv.length}\n🆒 Background Size: ${xbground} x ${ybground}\n🆕Avatar Size: ${s}${mode}\n#️⃣Color: ${color}\n⏳Processing your request, it may take up to 1 minute to complete...`,
      threadID,
      messageID
    );
    var loadkhung = await Canvas.loadImage(
      "https://i.ibb.co/H41cdDM/1624768781720.png"
    ); //("https://s1.uphinh.org/2021/06/24/1624551553171.png");
    ...
    for (let id of idtv) {
      ...
      try {
        var avatar = await superfetch.get(
    ...
    try {
      //ktra auto cắt ảnh có bị lỗi hay ko
      ...
      api.sendMessage(
        {
          body: `🟦Number of photos: ${dem} (Filtered ${ngdung} Facebook users)\n🆒 Background Size: ${xbground} x ${ybground}\n🆕Avatar Size: ${s}${mode}\n⏱️Processing Time: ${Math.floor(
            (Date.now() - timestart) / 1000
          )} second`,
          attachment: fs.createReadStream(path_alltv, {
