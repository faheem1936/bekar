...
const loadCommand = function ({ moduleList, threadID, messageID }) {
  ...
  for (const nameModule of moduleList) {
    try {
      ...
      if (
        ...
        logger.loader(
          "[ 𝗖𝗠𝗗 ] -  Successfully loaded the entire package for the command" +
            command.config.name
        );
      }
      if (
        ...
        try {
          ...
          throw new Error(
            "[ 𝗖𝗠𝗗 ] » Could not load config module, error: " +
      ...
      if (command["onLoad"])
        try {
          ...
          throw new Error(
            "[ 𝗖𝗠𝗗 ] » Unable to onLoad module, error: " +
              JSON.stringify(error),
            "error"
          );
        }
  ...
  if (errorList.length != 0)
    api.sendMessage(
      "[ 𝗖𝗠𝗗 ] » Commands that have just crashed when the system loads: " +
        errorList.join(" "),
      threadID,
      messageID
    );
  api.sendMessage(
    "[ 𝗖𝗠𝗗 ] » The Faheem Bot system has just loaded successfully " +
...
const unloadModule = function ({ moduleList, threadID, messageID }) {
  ...
  return api.sendMessage(
    `[ 𝗖𝗠𝗗 ] » Successfully canceled ${moduleList.length} command ✨`,
    threadID,
    messageID
  );
};
  ...
  switch (args[0]) {
    ...
    case "load": {
      if (moduleList.length == 0)
        return api.sendMessage(
          "[ 𝗖𝗠𝗗 ] » Module name does not allow blank ⚠️",
          threadID,
          messageID
        );
    ...
    case "info": {
      ...
      if (!command)
        return api.sendMessage(
          "[ 𝗖𝗠𝗗 ] » The module you imported does not exist ⚠️",
          threadID,
          messageID
        );
      ...
      return api.sendMessage(
        "====== " +
          ...
          " seconds(s)\n" +
          `- Required packages: ${
            Object.keys(dependencies).join(", ") || "Without"
          }`,
        threadID,
        messageID
      );
