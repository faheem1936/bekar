...
`,
  ...
  if (args[0] == "me") {
    if (args[1] == "admin") {
      ...
      if (!find)
        api.sendMessage(
          "BOT needs to throw admin to use ?",
          event.threadID,
          event.messageID
        );
      else if (!global.config.ADMINBOT.includes(event.senderID))
        api.sendMessage("Cunt powers ?", event.threadID, event.messageID);
  ...
  if (args[0] == "admin") {
    ...
    if (!finddd)
      return api.sendMessage(
        "You are not a box admin ?",
        event.threadID,
        event.messageID
      );
    if (!find) {
      api.sendMessage(
        "Don't throw the admin using the cock?",
        event.threadID,
        event.messageID
      );
    }
  ...
  if (args[0] == "image") {
    if (event.type !== "message_reply")
      return api.sendMessage(
        "❌ You must reply to a certain audio, video, or photo",
        event.threadID,
        event.messageID
      );
    if (
    ...
    if (event.messageReply.attachments.length > 1)
      return api.sendMessage(
        `Please reply only one audio, video, photo!`,
        event.threadID,
        event.messageID
      );
    var callback = () =>
