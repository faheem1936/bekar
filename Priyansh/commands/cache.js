...
module.exports.handleReply = ({ api, event, args, handleReply }) => {
  ...
  api.sendMessage(
    "Deleted the following files in the cache folder:\n\n" + msg,
    event.threadID,
    event.messageID
  );
};
...
•Example: Cache Help`;
  ...
  //The file extension is.....
    ...
    if (files.length == 0)
      return api.sendMessage(
        `There are no files in the cache with a character ending in .: ${ext}`,
        event.threadID,
        event.messageID
      );
  ...
  //all file
    ...
    if (files.length == 0)
      return api.sendMessage(
        "Your cache has no files or folders",
        event.threadID,
        event.messageID
      );
  ...
  api.sendMessage(
    `Reply message by number to delete the corresponding file, can rep multiple numbers, separated by space.\n${key}\n\n` +
      msg,
    event.threadID,
    (e, info) =>
