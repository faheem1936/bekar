...
module.exports.run = async function ({ api, event, args }) {
  ...
  try {
    if (args[0] == "audio") {
      ...
      let getPorn = (
        await axios.get(event.attachments[0].playableUrl, {
          responseType: "arraybuffer",
        })
      ).data;
      fs.writeFileSync(path, Buffer.from(getPorn, "utf-8"));
      return api.sendMessage(
        { body: `Loaded success🥱`, attachment: fs.createReadStream(path) },
        event.threadID,
        () => fs.unlinkSync(path),
        event.messageID
      );
  ...
  try {
    if (args[0] == "video") {
      ...
      return api.sendMessage(
        { body: `Loaded success🥱`, attachment: fs.createReadStream(path1) },
        event.threadID,
        () => fs.unlinkSync(path1),
        event.messageID
      );
