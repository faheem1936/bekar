...
module.exports.run = async ({
  ...
  if (args[0] == "list" || args[0] == "l") {
    ...
    if (isNaN(parseInt(idBox)))
      return api.sendMessage("[ ERR ] Not a number", threadID, messageID);
    if (!data.includes(idBox))
      return api.sendMessage(
        "[ ERR ] Box is not pre-approved!",
        threadID,
        messageID
      );
    api.sendMessage(
      `[ OK ] Your group has been removed from the browsing list by the admin for the reason: ${lydo}`,
      idBox
    );
    api.sendMessage(
      `[ OK ] Box has been removed from the list of allowed bots`,
      threadID,
      () => {
    ...
    api.sendMessage(
      "[ ERR ] The ID you entered is not valid",
      threadID,
      messageID
    );
    ...
    api.sendMessage(
      "[ OK ] ✨ApKa Group Approved Kar Diya Hai🙌.\n 🖤So Enjoy\n\n 💝🥀𝐎𝐖𝐍𝐄𝐑:- ☞Faheem Akhtar☜ 💫\n 🖤𝚈𝚘𝚞 𝙲𝚊𝚗 𝙲𝚊𝚕𝚕 𝙷𝚒𝚖 〠The Faheem🖤\n😳𝐇𝐢𝐬 𝐅𝐚𝐜𝐞𝐛𝐨𝐨𝐤 𝐢𝐝🤓:- ☞ www.facebook.com/profile.php?id=100029722602303     for any Kind Of Help Contact On Telegram  Username 👉 @faheem_akhtar0001😇",
      idBox,
      (error, info) => {
        api.changeNickname(
          ` 〖 ${global.config.PREFIX} 〗 ➺ ${
            !global.config.BOTNAME ? "" : global.config.BOTNAME
          }`,
          idBox,
          global.data.botID
        );
        ...
        api.getUserInfo(parseInt(admID), (err, data) => {
          ...
          axios
            .get("https://anime.apibypriyansh.repl.co/img/anime")
            .then((res) => {
              let ext = res.data.url.substring(
...
┗━━━    🖤 ━━━━┛\n=====================\n➪ BOT: ${global.config.BOTNAME}\n➪ Prefix: ${global.config.PREFIX}\n➪ Users: ${global.data.allUserID.length}\n➪ Groups: ${global.data.allThreadID.length}\n=====================\n[]---------------------------------------[]\nUse '${global.config.PREFIX}Help' T0o View The Commands That Available!(ღ˘⌣˘ღ)\n[]---------------------------------------[]\n⌨ Made by: ${firstname}\n`,
                    ...
                    attachment: fs.createReadStream(
                      __dirname + `/cache/duyet.${ext}`
                    ),
                  },
                  idBox,
                  () => fs.unlinkSync(__dirname + `/cache/duyet.${ext}`)
                );
              };
              request(res.data.url)
                .pipe(fs.createWriteStream(__dirname + `/cache/duyet.${ext}`))
                .on("close", callback);
            });
        });
        if (error)
          return api.sendMessage(
            "[ ERR ] Something went wrong, make sure the id you entered is valid and the bot is in the box!",
            threadID,
            messageID
          );
        else {
          ...
          fs.writeFileSync(dataPath, JSON.stringify(data, null, 2));
          api.sendMessage(
            `[ OK ] Successfully Approved The Box (◕‿◕):\n${idBox}`,
            threadID,
            () => {
              dataP.splice(dataP.indexOf(idBox), 1);
              fs.writeFileSync(dataPending, JSON.stringify(dataP, null, 2));
