...
module.exports.handleReply = async ({ handleReply, event, api }) => {
  ...
  if (args[0] == "add") {
    ...
    return api.sendMessage(
      'Please choose <add | del > <numerical order | or "all">',
      event.threadID,
      event.messageID
    );
  ...
  api.sendMessage(
    `» Đã ${
      args[0] == "add" ? "accept" : "erase"
    } successful friend request of ${success.length} People:\n${success.join(
      "\n"
    )}${
      failed.length > 0
...
module.exports.run = async ({ event, api }) => {
  ...
  api.sendMessage(
    `${msg}\nReply to this message with content: <add | del> <numerical order | or \"all\"> to take action`,
    event.threadID,
    (e, info) => {
