...
module.exports.run = async ({
  ...
  const selectedLink = link[randomIndex];
  ...
  const imagePath = path.join(cacheDir, "meme.jpg");

  var callback = () => {
    api.sendMessage(
      {
        body: `Random Meme 😂\nMade by Faheem\nTotal memes: ${link.length}`,
  ...
  return request(encodeURI(selectedLink))
    ...
    .on("error", (err) => {
      ...
      api.sendMessage(
        "Sorry, there was an error downloading the meme. Please try again.",
        event.threadID,
        event.messageID
      );
    });
