module.exports.config = {
  ...
  description:
    "Permanently ban members from the group (Remember to set the qtv bot)\nAuthor: 𝐏𝐫𝐢𝐲𝐚𝐧𝐬𝐡 𝐑𝐚𝐣𝐩𝐮𝐭",
  ...
  info: [
    ...
    },
    {
      key: "view",
      prompt:
        '"tag" or "blank" or "view all", respectively used to see how many times the person tagged or yourself or a member of the box has been warned ',
      type: "",
...
module.exports.run = async function ({
  ...
  if (!info.adminIDs.some((item) => item.id == api.getCurrentUserID()))
    return api.sendMessage(
      "The bot needs group admin rights to use this command\nPlease add and try again!",
      threadID,
      messageID
    );
  ...
  */
  ...
  if (args[0] == "view") {
    if (!args[1]) {
      ...
      api.sendMessage(
        `❎You have been warned for the reason : ${msg}`,
        threadID,
        messageID
      );
      ...
      allwarn == ""
        ? api.sendMessage(
            "✅No one in your group has been warned yet",
            threadID,
            messageID
          )
        : api.sendMessage(
    ...
    var info = await api.getThreadInfo(threadID);
    if (
      !info.adminIDs.some((item) => item.id == senderID) &&
    ...
    if (!id)
      return api.sendMessage(
        "❎Need to enter the id of the person to be removed from the banned list of the group",
        threadID,
        messageID
      );
    bans.banned;
    if (!mybox.includes(id))
      return api.sendMessage(
        "✅This person hasn't been banned from your group yet",
        threadID,
        messageID
      );
    api.sendMessage(
      `✅Removed the member with id ${id} from the group banned list`,
      threadID,
      messageID
    );
    ...
    msg == ""
      ? api.sendMessage(
          "✅No one in your group has been banned from the group yet",
          threadID,
          messageID
        )
      : api.sendMessage(
          "❎Members who have been banned from the group:\n" + msg,
          threadID,
          messageID
        );
  ...
  //◆━━━━━━━━━◆WARN◆━━━━━━━━━◆\\
    ...
    api.sendMessage(
      {
        body: `Banned members ${arrayname.join(
          ", "
        )} permanently leave the group for the reason: ${reason}`,
        mentions: arraytag,
      },
      threadID,
      messageID
